<%@ page contentType="text/html;charset=utf-8" import="java.util.ArrayList, land.mvc.domain.Landmark, jeju.all.consts.BoardConst"%>
...
<center>
      ...
      <center>
         ...
         <form name="paging" action="land.do?m=list" method="post" style="margin-left:-90; margin-top: 130; margin-bottom:-50;">
...
<c:if test="${empty list }">
	     <tr>
				<td colspan="5" style="text-align:center">관광지가 하나도 없네요</td>
		</tr>
</c:if>
      <% 
      ...
      <div align="center" style="margin-bottom:-80;">      
         <tr>
            <td colspan="3" align="center">
               <a id="back" style="display:inline-block" href="land.do?m=list&cp=<%=cp-1%>&ps=<%=ps%>">◀이전</a>
                      |
                 <a id="page1" href="land.do?m=list&cp=<%=cpListTemp.get(0)%>&ps=<%=ps%>"><%=cpListTemp.get(0)%></a>
                 <a id="page2" style="display:inline-block" href="land.do?m=list&cp=<%=cpListTemp.get(1)%>&ps=<%=ps%>"><%=cpListTemp.get(1)%></a>
                 <a id="page3" style="display:inline-block" href="land.do?m=list&cp=<%=cpListTemp.get(2)%>&ps=<%=ps%>"><b><%=cpListTemp.get(2)%></b></a>
               <a id="page4" style="display:inline-block" href="land.do?m=list&cp=<%=cpListTemp.get(3)%>&ps=<%=ps%>"><%=cpListTemp.get(3)%></a>
               <a id="page5" style="display:inline-block" href="land.do?m=list&cp=<%=cpListTemp.get(4)%>&ps=<%=ps%>"><%=cpListTemp.get(4)%></a>
                  |
                 <a id="front" style="display:inline-block" href="land.do?m=list&cp=<%=cp+1%>&ps=<%=ps%>">이후▶</a>
               &nbsp;&nbsp;&nbsp;
...
<c:forEach items="${list}" var="land">
   ...
   <!--  <ul class="row" style="width: 1500px;" align="center">
		...
		&nbsp;&nbsp;&nbsp;&nbsp;
            <div class="img-box" ><img src="../photo/${land.lphoto}" width="400" height="400" alt=""></div>
            <a href='land.do?m=content&lnum=${land.lnum}'>${land.lname}<input type="submit" value="상세내용"></a>
            <div class="">${land.lintro}</div>
            <div class="">평점 : ${land.star}</div>
            		&nbsp;&nbsp;&nbsp;&nbsp;		
		</div>
    	</ul>  -->
         ...
         <div class="cardbox shadow-lg bg-white">
          ...
          <a href='land.do?m=content&lnum=${land.lnum}'>${land.lname}</a>
          
          </font>
         
         <div style="height: auto; width: 100%;  border-top:2px solid silver;">
            <div class="" style="margin-top:5">${land.lintro}</div>
            <div class="">평점 : ${land.star}</div>
